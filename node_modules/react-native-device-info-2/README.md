...
[![npm version](https://badge.fury.io/js/react-native-device-info-2.svg)](http://badge.fury.io/js/react-native-device-info-2)
...
```shell
npm install --save react-native-device-info-2
```

or using yarn:

```shell
yarn add react-native-device-info-2
```
...
```shell
rnpm link react-native-device-info-2
```
...
`pod 'RNDeviceInfo', :path => '../node_modules/react-native-device-info-2'`
...
* Go to `node_modules/react-native-device-info-2`
...
+   compile project(':react-native-device-info-2')
}
```
...
...
include ':app'
+ include ':react-native-device-info-2'
+ project(':react-native-device-info-2').projectDir = new File(rootProject.projectDir, '../node_modules/react-native-device-info-2/android')
```
...
* Navigate to `./<app-name>/node_modules/react-native-device-info-2/windows/RNDeviceInfo` and add `RNDeviceInfo.csproj`
...
// or import DeviceInfo from 'react-native-device-info-2';
```
...
When installing or using `react-native-device-info-2`, you may encounter the following problems:

<details>
  <summary>[android] - Unable to merge dex / Multiple dex files / Problems with `com.google.android.gms`</summary>

`react-native-device-info-2` uses `com.google.android.gms:play-services-gcm` to provide [getInstance()][#getinstance].
...
`googlePlayServicesVersion` gradle variable in your `build.gradle` file to tell `react-native-device-info-2` what version
it should require.
...
`react-native-device-info-2` contains native code, and needs to be mocked.
...
// testenv.js:
jest.mock('react-native-device-info-2', () => {
  return {
    getModel: jest.fn(),
  };
});
...
See the [CHANGELOG.md](https://github.com/rebeccahughes/react-native-device-info-2/blob/master/CHANGELOG.md).
